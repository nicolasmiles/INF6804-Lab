...
def track_MOT(scale=0.25, box_size_evolution=1, particle_movement=20, nb_particles=200, color_histogram=True, gradient_histogram=True):
    bbox_list = tracker_particle(frame_list, [person1_bbox], printing=False, scale=scale,
                                 box_size_evolution=box_size_evolution, particle_movement=particle_movement,
                                 nb_particles=nb_particles, color_histogram=color_histogram,
...
for epoch in range(20):
    ...
    score = track_MOT(scale=scale, box_size_evolution=box_size_evolution, particle_movement=particle_movement, nb_particles=nb_particles, color_histogram=color_histogram, gradient_histogram=gradient_histogram)
    print(f"Score = {score}")
    score_file = "score_orb.txt"
    if color_histogram and gradient_histogram:
        score_file = "score_hists.txt"
    if color_histogram:
        score_file = "score_color_hist.txt"
    if gradient_histogram:
        score_file = "score_gradient_hist.txt"
